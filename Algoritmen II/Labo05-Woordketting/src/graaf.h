...
public:
    GraafExceptie(const std::string &boodschap_)
            : std::logic_error(boodschap_) {
    }
};
...
public:
    ...
    // zoekt al de kettingen in deze graaf.
    Graaf<RT> zoek_kettingen(int begin_knoop);

    std::stack<int> diepte_eerst_zoeken(int begin_knoop);
    void behandel(int knoop_id, std::vector<Kleur>& kleuren, std::stack<int>& pad);
...
void Graaf<RT>::controleerKnoopnummer(int k) const {
    if (k < 0 || (size_t) k >= knopen.size()) {
        ...
        out << "Ongeldig knoopnummer " << k << ". Moet >= 0 en < " << knopen.size()
                << " zijn.";
...
template<RichtType RT>
Graaf<RT>::Graaf(int n)
        : knopen(n), hoogsteVerbindingsnummer(0) {
}
;
...
int Graaf<RT>::aantalKnopen() const {
    std::cout << "Aantal knopen: " << knopen.size() << std::endl;
    return knopen.size();
}
...
void GraafMetKnoopdata<RT, Knoopdata>::schrijfKnoop(std::ostream &os, int k) const {
    ...
    for (std::map<int, int>::const_iterator it = this->knopen[k].begin();
            it != this->knopen[k].end(); ++it) {
        os << "  ->" << it->first;
...
public:
    ...
    void zoek_kettingen(int begin_knoop);

    GraafMetKnoopEnTakdata<RT, Knoopdata, Takdata> keerom();
};

template<RichtType RT, class Knoopdata, class Takdata>
GraafMetKnoopEnTakdata<RT, Knoopdata, Takdata> GraafMetKnoopEnTakdata<RT, Knoopdata,
        Takdata>::keerom() {
    ...
    std::cout << "De graaf is omgekeerd" << std::endl;
